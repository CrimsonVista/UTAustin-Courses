...
class ProxyDataProtocol(asyncio.Protocol):
    ...
    def connection_made(self, transport):
        self.spoke.debug("connection made to {}".format(transport.get_extra_info("peername")))
...
class NetworkClassroomSpoke(asyncio.Protocol):
    ...
    def handle_push(self, header, payload):
        ...
        if h["command"] == "connect":
            ...
            if proxy_key not in self.proxy_conns or self.proxy_conns[proxy_key].transport is None:
                ...
                self.debug("Spoke transmitting {} bytes to proxy".format(len(payload)))
    ...
    def data_received_unsafe(self, data):
        self.debug("Received {} bytes of data.".format(len(data)))
        ...
        while data:
            ...
            if h == None:
                self.buffer += data
                return
            self.debug("Unpacked message with header:\n{}\nAnd {} bytes of payload.".format(h, len(payload)))
