        ...
        while data:
            ...
            if h["command"] == "push_response":
                ...
                if push["command"] == "connect":
                    ...
                    if connecting_obj is None:
                        ...
                        print("Success opening connection.")
            ...
            if h["command"] == "LOGIN_USER":
                ...
                if this_conn != self:
                    ...
                    print("Proxy Data.", conn_user, conn_id, direction)
                    
                    conn_obj, conn_servers, conn_conns = self.users.get(conn_user, (None, None, None))
                    if conn_obj is None or conn_id not in conn_conns:
                        response["error"] == "Unknown connection"
    ...
    def connection_lost(self, reason):
        if self.connected_user and self.connected_user in self.users:
            ...
            while conns:
                ...
                if (self.connected_user, conn_id) in conns:
                    del conns[(self.connected_user, conn_id)]
...
class NetworkClassroomHubController:
    ...
    def list_user_data(self, writer):
        for username in self.hub.users:
            ...
            for (conn_user, conn_id) in conns:
                remote_user, remote_port = conns[(conn_user, conn_id)]
...
if os.path.exists("key.pem"):
    ...
    ssl_context=None
coro = loop.create_server(hub.build, port=34150, ssl=ssl_context)
