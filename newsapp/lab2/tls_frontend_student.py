...
class TLSSession:
    ...
    def _derive_keys(self):
        ...
        start = 0
        end = self.mac_key_size
        self.read_mac = key_block[start:end]
        start = end
        end += self.mac_key_size
        self.write_mac = key_block[start:end]
        start = end
        end += self.enc_key_size
        self.read_enc = key_block[start:end]
        start = end
        end += self.enc_key_size
        self.write_enc = key_block[start:end]
        # start = end
        # end += self.iv_size
        # self.read_iv = key_block[start:end]
        # start = end
        # end += self.iv_size
        # self.write_iv = key_block[start:end]
        assert(end == self.key_block_len)
    ...
    def decrypt_tls_pkt(self, tls_pkt, **kargs):
        ...
        if kargs is not None and 'is_handshake' in kargs and kargs['is_handshake']:
            packet_type = 22
        ...
        """
        contentType = tls_pkt_bytes[0]
        version = tls_pkt_bytes[1:3]

        tls_pkt_bytes = tls_pkt_bytes[5:]
        iv = tls_pkt_bytes[:self.iv_size]
        ciphertext_bytes = tls_pkt_bytes[self.iv_size:]
        ...
        padding_length = plaintext_bytes[-1] + 1
        data = plaintext_bytes[:-1 * padding_length]
        content_bytes = data[:-20]
        msg = struct.pack("!Q", self.read_seq_num) + struct.pack("!B", contentType) + version + struct.pack("!H", len(content_bytes)) + content_bytes
        self.read_seq_num += 1

        # check hmac
        hmac = data[-20:]
        h = crypto_hmac.HMAC(self.read_mac, hashes.SHA1(), default_backend())
        h.update(msg)
        h.verify(hmac)
    ...
    def encrypt_tls_pkt(self, tls_pkt):
        ...
        contentType = tls_pkt_bytes[0]
        version = tls_pkt_bytes[1:3]
        msg = struct.pack("!Q", self.write_seq_num) + struct.pack("!B", contentType) + version + struct.pack("!H", len(plaintext_bytes)) + plaintext_bytes

        h = crypto_hmac.HMAC(self.write_mac, hashes.SHA1(), default_backend())
        h.update(msg)
        hmac = h.finalize()
        ...
        data = plaintext_bytes + hmac
        block_size = self.iv_size
        padding_length = block_size - len(data) % block_size - 1
        padding = struct.pack("!B",padding_length) * padding_length

        ct = encryptor.update(data + padding + struct.pack("!B",padding_length)) + encryptor.finalize()
        content = iv + ct
        return struct.pack("!B", contentType) + version + struct.pack("!H", len(content)) + content
    ...
    def compute_handshake_verify(self, mode):
        ...
        """
        return self.PRF.compute_verify_data("server", mode, self.handshake_messages, self.master_secret)
